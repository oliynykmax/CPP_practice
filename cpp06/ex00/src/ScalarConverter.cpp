...
static const std::regex& doubleRegex() {
    static const std::regex re(
        std::string("^") + RE_SIGN + RE_DEC_FRAC + RE_EXP + "$"
    );
    return re;
}
...
static const std::regex& floatRegex() {
    static const std::regex re(
        std::string("^") + RE_SIGN + RE_DEC_FRAC + RE_EXP + "[fF]$"
    );
    return re;
}
...
static void print_converted(std::string n, double d, float f) {
    ...
    if (n.empty()) std::cout << "int: " << static_cast<int>(d) << std::endl;
    else std::cout << "int: " << n << std::endl;
