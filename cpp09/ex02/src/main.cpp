...
int main(int argc, char **argv) {
    ...
    std::cout << "Time to process a range of " << result_vec.size() << " elements with std::vector : "
              << std::fixed << std::setprecision(5)
              << std::chrono::duration_cast<std::chrono::nanoseconds>(end_vec - start_vec).count() / 1000.0
              << " us" << std::endl;
    std::cout << "Time to process a range of " << result_vec.size() << " elements with std::deque : "
              << std::fixed << std::setprecision(5)
              << std::chrono::duration_cast<std::chrono::nanoseconds>(end_deq - start_deq).count() / 1000.0
              << " us" << std::endl;
}
